...
def clean_data(raw_data, min_data_points=200):
    ...
    if isinstance(raw_data.columns, pd.MultiIndex):
        ...
        # Bug fix: single-ticker flat DataFrame — derive tickers from columns,
        # excluding standard OHLCV field names instead of relying on columns.name
        ohlcv_fields = {'Open', 'High', 'Low', 'Close', 'Volume',
                        'Adj Close', 'log_returns', 'volatility'}
        derived = [c for c in raw_data.columns if c not in ohlcv_fields]
        tickers = derived if derived else list(raw_data.columns)
    ...
    if valid_tickers:
        cleaned_data = raw_data[valid_tickers]
